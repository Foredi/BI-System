...
tables = ['Customer_Dimension', 'Product_Dimension', 'Supplier_Dimension', 'Time_Dimension', 'Location_Dimension', 'Sales_Fact']
# tên các bảng trong Qdrant
collections = ['Customer_Dimension', 'Product_Dimension', 'Supplier_Dimension', 'Time_Dimension', 'Location_Dimension', 'Sales_Fact']
...
def insert_data_postgres():
    try:
        for table in tables:
            df = pd.read_csv(f"/opt/airflow/dags/star/{table}.csv")
            if not df.empty:
...
with DAG('pipeline_star', default_args=default_args, schedule_interval='0 * * * *') as dag:
    ...
    start  >> insert_data_postgres >> end
